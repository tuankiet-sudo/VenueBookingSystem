...
function VenueDetailPage() {
  ...
  // Booking form state - Initialize from store
  ...
  const [capacity, setCapacity] = useState(searchParams.capacity || 0);

  useEffect(() => {
    console.log('[VenueDetailPage] Initialized from store:', {
      searchParams,
    });
  ...
  const fetchVenueDetail = useCallback(async () => {
    setIsLoading(true);
    try {
      // Mock data - find venue by combining location_id and name as ID
      await new Promise((resolve) => setTimeout(resolve, 500));

      console.log('Looking for venue ID:', venueId);

      const foundVenue = mockVenues.find((v) => {
        // Create a unique ID from location_id and venue name
        const uniqueId = `${v.location.location_id}-${v.name.replace(/\s+/g, '-')}`;
        console.log('Checking venue:', v.name, 'Generated ID:', uniqueId);
        return uniqueId === venueId;
      });

      console.log('Found venue:', foundVenue?.name || 'NOT FOUND');

      if (foundVenue) {
        setVenue(foundVenue);
        setCapacity(foundVenue.venueType.minCapacity);
  ...
  const handleBookNow = () => {
    ...
    console.log('[VenueDetailPage] handleBookNow - Saving to store:', {
    ...
    // Save booking params to store
    setSearchParams({
      location: venue.location.name,
      checkIn: new Date(bookingDate),
      checkOut: new Date(bookingDate),
      startTime: startTime,
      endTime: endTime,
      capacity: capacity,
      rooms: 1,
              ...
              <button
                onClick={() =>
                  navigate({
                    ...
                    params: { 'location-id': venue.location.location_id },
                  })
                }
                className="hover:text-primary"
              >
                ...
                <div className="mb-6 space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          console.log(
                            '[VenueDetailPage] Start time changed:',
                            e.target.value,
                          );
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          console.log(
                            '[VenueDetailPage] End time changed:',
                            e.target.value,
                          );
                  ...
                  <div>
                    ...
                    <input
                      ...
                      min={venue.venueType.minCapacity}
                      max={venue.venueType.maxCapacity}
                      className="w-full rounded-lg border border-gray-300 p-2.5 text-sm focus:border-primary focus:outline-none"
                    />
                    <p className="mt-1 text-xs text-gray-500">
                      Min: {venue.venueType.minCapacity}, Max:{' '}
                      {venue.venueType.maxCapacity}
                    </p>
                  </div>
                </div>
